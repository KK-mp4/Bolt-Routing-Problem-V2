...
// Let's try async...
export async function generateNNGraphASYNC(
    network: Network,
    callback: (path: Network) => void
): Promise<void> {
    network.bolts = []
    const stations = network.stations
    const numStations = stations.length
    ...
    for (let i = 0; i < numBolts; ++i) {
        for (let j = i + 1; j < numBolts; ++j) {
            ...
            if (intersects(boltA, boltB)) {
                ...
                console.log(
                    boltA.station_a.name +
                        ' => ' +
                        boltA.station_b.name +
                        ' intersects with ' +
                        boltB.station_a.name +
                        ' => ' +
                        boltB.station_b.name
                )
