    ...
    for (let startIdx = 0; startIdx < numStations; startIdx++) {
        ...
        for (let count = 0; count < numStations - 1; ++count) {
            ...
            for (let i = 0; i < numStations; ++i) {
                if (stations[minKeyIndex] && stations[i]) {
                    // Check if both stations are defined
        ...
        for (let i = 1; i < numStations; ++i) {
            if (stations[parent[i]] && stations[i]) {
                // Check if both stations are defined
                const distance = chebyshevDistance(
                    stations[parent[i]],
                    stations[i]
                ...
                bolts.push({
                    directed: false,
                    station_a: {
                        name: stations[parent[i]].name,
                        x: stations[parent[i]].x,
                        z: stations[parent[i]].z,
                    },
        ...
        plotData.value.push({
            graph_name: "Prim's from " + network.stations[startIdx].name,
            length: totalDistance,
            time: calculateAverageTravelTime({
